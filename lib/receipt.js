...
export async function buildReceipt(data) {
  ...
  try {
    const logoImg = await loadImage("/menu/logo_newcam.png"); 
    result = result.image(logoImg, 192, 192, "atkinson").newline();
  } catch (err) {
    console.warn("Failed to load logo for receipt", err);
  }

  // 2. TEKS: Setel ke kiri secara hardware, dorong ke tengah secara software
  result = result.align("left");

  // Header
  result = result
    .bold(true)
    .line(centerText("COFFEE NEW CAMMARY", MAX_CHAR))
    .bold(false)
    .line(centerText("Jl. Manisi, Cibiru, Bandung", MAX_CHAR))
    .line(lineChars);
  ...
  result = result
    .bold(true)
    .line(centerText('"Pertemuan Adalah Kabar"', MAX_CHAR))
    .bold(false)
    .line(centerText("Wi-Fi: udahnyambungbelum", MAX_CHAR))
    .newline()
    .line(centerText("Follow Atuh Euy", MAX_CHAR))
    .line(centerText("@coffee_newcammary", MAX_CHAR))
    .newline()
    .newline()
    .cut();
